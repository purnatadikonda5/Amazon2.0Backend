 ...
 * 
 * WHY USE THIS:
 * This listens aggressively to the actual CloudAMQP WebSocket for data
 * streaming down!
 * You no longer need @Async because Spring automatically allocates an
 * independent
...
public class PaymentRabbitListener {
    ...
    public void handlePaymentSuccessEvent(PaymentSuccessEvent event, com.rabbitmq.client.Channel channel,
            org.springframework.messaging.Message<?> message) throws java.io.IOException {
        log.info("🔥 RABBITMQ WORKER TRIGGERED: Starting robust tasks for Order ID: {} from the CloudAMQP Server!",
                event.getOrderId());
        ...
        if (Boolean.FALSE.equals(isNew)) {
            log.warn("⚠️ Idempotency Triggered! Order {} was already processed. Silently acknowledging duplicate.",
                    event.getOrderId());
